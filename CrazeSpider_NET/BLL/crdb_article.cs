		...
		public List<CRDB.Model.crdb_article> DataTableToList(DataTable dt)
		{
			...
			if (rowsCount > 0)
			{
				...
				for (int n = 0; n < rowsCount; n++)
				{
					...
					if(dt.Rows[n]["bloom_offset1"].ToString()!="")
					{
						model.bloom_offset1=int.Parse(dt.Rows[n]["bloom_offset1"].ToString());
					}
					if(dt.Rows[n]["bloom_offset2"].ToString()!="")
					{
						model.bloom_offset2=int.Parse(dt.Rows[n]["bloom_offset2"].ToString());
					}
					modelList.Add(model);
				}
			}
